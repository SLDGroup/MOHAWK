...
class EfficientNet(nn.Module):
    def __init__(self, cfg, num_classes=10, channels=3, initializer=None):
    ...
    def _make_layers(self, in_channels):
        ...
        for expansion, out_channels, num_blocks, kernel_size, stride in zip(*cfg):
            ...
            for stride_ in strides:
                ...
                layers.append(
                    Block(in_channels, out_channels, kernel_size, stride_,
    ...
    def forward(self, x):
        ...
        if self.training and dropout_rate > 0:
            out = F.dropout(x_out, p=dropout_rate)
            out = self.linear(out)
        else:
...
def EfficientNetB0(num_classes=10, channels=3, initializer=None):
    cfg = {
    ...
    return EfficientNet(cfg, num_classes=num_classes, channels=channels, initializer=initializer)
#
# if __name__=="__main__":
#     model = EfficientNetB0(num_classes=10, channels=3, initializer=kaiming_normal)
